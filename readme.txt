...
= 1.9.5 =
* 修正: 他プラグイン/テーマの dashboard_glance_items コールバックが null を返すサイトで、概要ウィジェットが致命的エラー(TypeError)になる問題を修正(受け取った値を配列に正規化)
* 修正: 推移グラフの代替テキスト(aria-label)が期間選択にかかわらず「過去14日」固定だったのを、選択中の日数に追従するよう修正

= 1.9.4 =
* 推移グラフのツールチップを WP Mail SMTP 同等の見た目・配置に調整
  - ダークスタイル(黒地・白文字・角丸)に変更、系列マーカーを黒地でも視認できる色付きスクエアに
  - 表示位置をポイントの真横に変更(マウスに近い系列のポイント基準・右横、右端では左横に自動反転・上下はポイント中心揃え)

= 1.9.3 =
* 推移グラフのホバー挙動を WP Mail SMTP 同等のポイント基準に改善
  - データ点の丸を拡大(期間に応じ段階調整)し、ホバー中のポイントは強調リングで拡大表示
  - ツールチップは最寄りのポイントのすぐ上に表示(上に収まらない場合は下に回り込み)。画面上部固定をやめ、視線移動なく値を確認できるように

= 1.9.2 =
* 推移グラフのツールチップの表示位置を修正
  - グラフの下(一覧に重なる位置)に表示されていたのを、意図どおりグラフ上部に浮かぶよう修正(SVG 要素が offsetTop を持たないことに起因する位置計算の不具合)
...
= 1.9.0 =
* 推移グラフの集計期間セレクタを追加
  - サマリー行(受信トレイ | スパム)の右端にプルダウンを設置し、過去7日/14日/30日/90日を切り替え可能に
  - 選択はユーザーごとに記憶され、次回以降も維持される(nonce 検証つき)
* グラフの軸ラベルを読みやすく改善
  - 日付・件数の文字サイズを拡大(実表示約7px→約11px)、日付表記を「8/24」形式に変更
  - 期間に応じてラベルを自動間引き(最大7個程度)、長期間表示時はデータ点を小さく調整
